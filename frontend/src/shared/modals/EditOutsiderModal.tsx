...
type EditOutsiderModalProps = {
    onCancel: () => void,
    onEdit: (updatedOutsider: Outsider) => void,
    outsiderToEdit: Outsider
};

export default function EditOutsiderModal({ onCancel, onEdit, outsiderToEdit }: EditOutsiderModalProps) {
  ...
  useEffect(() => {
    setIsReady(false);
    if(outsiderToEdit) {
      const allowedDocTypes: (DocumentType | "Ninguno")[] = ["Ninguno", "DNI", "CE", "PAS"] as const

      const initDocType = allowedDocTypes.includes(outsiderToEdit.documentType!)
        ? (outsiderToEdit.documentType as DocumentType)
        : "Ninguno";
              
      const initDocId = outsiderToEdit.documentType === undefined ? "" : outsiderToEdit.documentId as string;

      //console.log("outToEdit:",outsiderToEdit.documentType);

      setDocumentType(initDocType);
      setDocumentId(initDocId);
      setName(outsiderToEdit.name);
      setLastName(outsiderToEdit.lastName);
      ...
      <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-100 h-full">
        <form onSubmit={handleConfirm} className="bg-[var(--bg-light-alt)] rounded-xl shadow-lg w-md sm:w-4/5 sm:max-w-2xl relative p-6">
          ...
          <h2 className="text-xl font-bold mb-4 text-[var(--brand)] dark:text-[var(--brand)] text-center">
            Modificar inscripción de externo
          </h2>
