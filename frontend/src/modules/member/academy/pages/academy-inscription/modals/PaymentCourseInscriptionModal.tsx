...
import { getInscriptionDescription, processCancellations, processNewInscriptions } from "../utils/CourseInscriptionOperations";
import { Button } from "@/components/ui/button";


type PaymentCourseInscriptionModalProps = {
    onClose: () => void
}

export default function PaymentCourseInscriptionModal({ onClose }: PaymentCourseInscriptionModalProps) {
    ...
    ), [course?.name, newInscriptions?.length])
    ...
    const handlePayment = useCallback(async () => {
        ...
        try {
            ...
            if(newInscriptions.length > 0)
                await processNewInscriptions(course, newInscriptions, newInscriptionsTotalCost, userId, inscriptionDescription, userFullname, userEmail);
    ...
    }, [account?.email, cancelledInscriptions, course, inscriptionDescription, navigate, newInscriptions, newInscriptionsTotalCost, user?.id, user?.lastname, user?.name, onClose]);
