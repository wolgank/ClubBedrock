...
type NewEventInscriptionPanelProps = {
    availableMembers: Member[],
    isInscriptionValid: (inscription: Member) => boolean,
    onAddInscription: (inscription: Member) => void,
    onRemoveInscription: (memberIdToRemove: number) => void
};

export default function NewEventInscriptionPanel({ availableMembers, isInscriptionValid, onAddInscription, onRemoveInscription } : NewEventInscriptionPanelProps) {
    const [memberSearchText, setMemberSearchText] = useState("");
    const [showMemberSuggestions, setShowMemberSuggestions] = useState(false);
    const [memberToRemove, setMemberToRemove] = useState<Member | null>(null);
    const { newInscriptions, newInscriptionsTotalCost, event } = useEventInscriptionContext();
    ...
    }, [isInscriptionValid, onAddInscription]);
        ...
        <Card className="background-custom rounded-[10px] shadow-lg w-full max-w-2xl p-6 flex flex-col justify-between h-[450px]">
            <div className="space-y-4">
                ...
                <div className="flex justify-between items-center">
                    ...
                    <p className="text-lg font-semibold text-[var(--brand-light)]">S/.{event.ticketPriceMember} por persona</p>
                </div>
                                ...
                                {newInscriptions.map((newMember) => (
                                    <li key={newMember.id} className="flex items-center justify-between p-2 border-gray-500 dark:border-gray-400">
                                        ...
                                        <div className="flex flex-nowrap gap-8">
                                            <button className="cursor-pointer" onClick={() => setMemberToRemove(newMember)}><X size={16} /></button>
                                        </div>
                                    </li>
                                ))}
                            </ul>
