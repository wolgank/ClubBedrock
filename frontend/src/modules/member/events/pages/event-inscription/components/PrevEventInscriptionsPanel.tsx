...
import { useEventInscriptionContext } from "./EventInscriptionContext";
import { Card } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { ArrowDown, ArrowUp, Info } from "lucide-react";
import { Tooltip, TooltipContent, TooltipTrigger } from "@/components/ui/tooltip";

type PrevEventInscriptionsPanelProps = {
    oldInscriptions: Member[],
    onCancelInscription: (inscription: Member) => void,
    onRecoverInscription: (inscription: Member) => void
}
...
export default function PrevEventInscriptionsPanel({ oldInscriptions, onCancelInscription, onRecoverInscription} : PrevEventInscriptionsPanelProps) {
    const [tabToShow, setTabToShow] = useState<'old' | 'cancelled'>('old');
    const { cancelledInscriptions } = useEventInscriptionContext();
                ...
                { tabToShow === 'old' ? (
                        <>
                            {oldInscriptions.length > 0 ? (
                                    <ul className="divide-y text-sm max-h-52 overflow-auto mt-2">
                                        {oldInscriptions.map((newMember) => (
                                            <li key={newMember.id} className="flex items-center justify-between p-2 border-gray-500 dark:border-gray-400">
                                                <div className="flex flex-col">
                                                    <span className="font-medium">{newMember.name} {newMember.lastname}</span>
                                                    <span className="text-gray-500 dark:text-gray-400 text-xs flex items-center gap-1">{newMember.memberType}</span>
                                                </div>
                                                <div className="flex flex-nowrap gap-8">
                                                    <button className="cursor-pointer" onClick={() => onCancelInscription(newMember)} title="Dar de baja inscripción"><ArrowDown/></button>
                                                </div>
                                            </li>
                                        ))}
                                    </ul>
                        ...
                        <>
                            {cancelledInscriptions.length > 0 ? (
                                    <ul className="divide-y text-sm max-h-52 overflow-auto mt-2">
                                        {cancelledInscriptions.map((newMember) => (
                                            <li key={newMember.id} className="flex items-center justify-between p-2 border-gray-500 dark:border-gray-400">
                                                <div className="flex flex-col">
                                                    <span className="font-medium">{newMember.name} {newMember.lastname}</span>
                                                    <span className="text-gray-500 dark:text-gray-400 text-xs flex items-center gap-1">{newMember.memberType}</span>
                                                ...
                                                <div className="flex flex-nowrap gap-8">
                                                    <button className="cursor-pointer" onClick={() => onRecoverInscription(newMember)} title="Recuperar inscripción"><ArrowUp/></button>
                                                </div>
                                            </li>
                                        ))}
                                    </ul>
