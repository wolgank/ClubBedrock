...
import { getSpace, disableSpaceById, getSpaceLeisure } from "@/lib/api/apiSpace"
...
export default function DataTable() {
    ...
    const deleteMutation = useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['get-Space-Leisure'] });
    ...
    const { isPending, error, data: dataSpace } = useQuery({
        queryKey: ['get-Space-Leisure'],
        queryFn: getSpaceLeisure,
    });
            ...
            cell: ({ row }) => (
                <div className="flex gap-2 items-center ">
                    <Button size="sm" className="button3-custom text-white" onClick={() => {
                        window.location.href = `/employee-event/espacios/${row.original.id}`;
                    }}>
                        Ver más
                    </Button>
                </div>
        ...
        <div className="w-full ">
            <div className="w-full flex items-center justify-between ">
                ...
                <Button className="h-full text-white font-bold rounded-lg border-0 button3-custom flex items-center"
                    onClick={() => exportTableToExcel(dataEvent, columns, "Espacios Leisure.xlsx")}>
